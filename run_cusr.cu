...
int main() {
    ...
    reg.fit(dataset, real_value, "pearson"); // do training
    ...
    // components
    for(auto &prog : reg.components) {
        cout << "component (in infix) :  " << cusr::program::prefix_to_infix(prog.prefix) << endl;
    }
    return 0;
}